...
class OrganisationReporter(BaseEntity):

    organisation = db.StringProperty(indexed=True)
    spreadsheet_id = db.StringProperty(indexed=True)
    sheet_name = db.StringProperty(indexed=False)
    end_row_number = db.IntegerProperty(indexed=False)

    def __init__(self, *args, **kwargs):
        super(OrganisationReporter, self).__init__(*args, **kwargs)
    ...
    @classmethod
    def _add_new(cls, organisation, spreadsheet_id, sheet_name, end_row_number=0):
        return OrganisationReporter(key_name=organisation, organisation=organisation, spreadsheet_id=spreadsheet_id, sheet_name=sheet_name, end_row_number=end_row_number)

    @classmethod
    def get_by_organisation(cls, organisation):
        """Load OrganisationReporter record by organisation name. Fail if organisation is not unique."""
        # Check OrganisationReporter entities for user_id: do a double look up here:
        # first by the key_name value and then by the organisation field value.
        org = OrganisationReporter.get_by_key_name(organisation)
        if org:
            return org

        orgs = OrganisationReporter.all().filter(
            OrganisationReporter.organisation.name, organisation).fetch(limit=2)
        if len(orgs) > 1:
            raise Exception(
                'There is more than one organisation reporter record with organisation name "%s"' % organisation)
