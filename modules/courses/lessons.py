...
#imports specifically for google sheets/org reporter
...
LEARNER_INST_TOKEN = 'learner_inst'

LEARNER_PRIVACY_TOKEN = 'learner_privacy'
LEARNER_PRIVACY_VALUE_DONT_SHARE = 'lp_dontshare'
LEARNER_PRIVACY_VALUE_ANONYMISE = 'lp_anonymise'
LEARNER_PRIVACY_VALUE_OK_SHARE = 'lp_oktoshare'

UNIT_NAMES = ['Mixing', 'Comparing', 'Scaling', 'Sharing', 'Trading off', 'Making decisions', 'Playing', 'Simulating', 'Interpreting data', 'Interpreting charts', 'Comparing groups', 'Appreciating', 'Tiling', 'Constructing', 'Reading scales', 'Converting', 'Estimating', 'Quantifying', 'End of course questionnaire']
...
class OrganisationReporterCronHandler(utils.AbstractAllCoursesCronHandler):

    URL = '/cron/org_reporter'
    ...
    def cron_action(self, app_context, global_state):
        if app_context.get_namespace_name() == 'ns_main':
            #set up orgs, course and progress tracker
            orgs = models.OrganisationReporter.all()
            course = courses.Course.get(app_context)
            progress_tracker = progress.UnitLessonCompletionTracker(course)

            #set up the service to update google sheets
            credentials = AppAssertionCredentials(GOOGLE_SHEETS_SCOPES)
            http_auth = credentials.authorize(Http())
            service = discovery.build('sheets', 'v4', http=http_auth, discoveryServiceUrl=GOOGLE_SHEETS_DISCOVERY_URL)

            #loop through the organisations
            for org in orgs:
                if org is not None:
                    org_id = org.organisation
                    spreadsheetId = org.spreadsheet_id
                    sheetName = org.sheet_name
                    endrow = org.end_row_number

                    logging.info('processing organisation: %s', org_id)

                    #clear the previous data, if any, from the target google sheet
                    if endrow >= 3:
                        clearBody = {}
                        clearRange = sheetName + '!A3:W' + str(endrow)
                        cleared = service.spreadsheets().values().clear(spreadsheetId=spreadsheetId, range=clearRange, body=clearBody).execute()

                    #start the inserting of data into the target google sheet
                    #using a cursor and limit on the students to stop the falling over of the student datastore when it reaches its batch timeout window, mainly because of the size of the student entity
                    row_number = 2
                    values = []
                    cursor = None
                    while True:
                        ...
                        fetched = 0
                        for student in query.run(limit = 500):
                            fetched += 1
                            additional_fields = student.additional_fields
                            #check if student is part of the organisation we are searching for
                            if student is not None and self.does_value_match(additional_fields, LEARNER_INST_TOKEN, org_id):
                                #check if the student has given permission to share data, whether anonymised or not, and then set the name for the student in variable name
                                lp = self.strip_value_from_additional_fields(additional_fields, LEARNER_PRIVACY_TOKEN)
                                if lp == LEARNER_PRIVACY_VALUE_OK_SHARE or lp == LEARNER_PRIVACY_VALUE_ANONYMISE:
                                    if lp == LEARNER_PRIVACY_VALUE_OK_SHARE:
                                        gn = self.strip_value_from_additional_fields(additional_fields, 'GivenName')
                                        fn = self.strip_value_from_additional_fields(additional_fields, 'FamilyName')
                                        name = gn + ' ' + fn
                                    else:
                                        name = 'Anonymised User'

                                    row_number += 1
                                    #add the name and formatted date times for enrolled on and last seen on to the row data list
                                    row_data = [name]
                                    row_data += [student.enrolled_on.strftime("%d-%b-%Y %X")]
                                    row_data += [student.last_seen_on.strftime("%d-%b-%Y %X")]

                                    #get the progress for the course for the student, and then add it to the row data list for each individual unit
                                    unit_percents = progress_tracker.get_unit_percent_complete_by_title(student)
                                    for un in UNIT_NAMES:
                                        row_data += [str(unit_percents[un] * 100) + '%']

                                    #check if the student has met the criteria to earn the certificate for the course, and add the relevant data to the row data list
                                    has_cert = certificate.student_is_qualified(student, course)
                                    if has_cert:
                                        row_data += ['Yes']
                                    else:
                                        row_data += ['No']

                                    #add the completed row data to the variable values which is a list of lists
                                    values.append(row_data)

                            #once we have reached the limit size, break out of the for loop and start the query again, this time getting the next installmant where the cursor has finished on for this current loop
                            if fetched == 500:
                                cursor = query.cursor()
                                break
                        else:
                    ...
                    #we know there is data being pushed to the google sheet if the row number is 3 or more. Set the range up, body and the update the google sheet with the data we have entered into values over the
                    #query limited for loop, and then set the end row number for the OrganisationReporter model to the row number we have counted up to. Set to 0 if no data is to be pushed to the google sheet.
                    if row_number >= 3:
                        rangeName = sheetName + '!A' + str(3) + ':W' + str(row_number)
                        body = {'values': values}
                        ...
                        org.end_row_number = row_number
                    else:
                        org.end_row_number = 0

                    #save the organisation record with the updated end row number
                    org.put()

                    logging.info('finished processing for organisation: %s', org_id)

    def does_value_match(self, additional_fields, token, to_match):
        match = False
        index = additional_fields.find(token)
        if index >= 0:
            ss = additional_fields[index:]
            end_index = ss.find('"]')
            if end_index > 0:
                value = ss[len(token) + 4:end_index]
                if value is not None and value == to_match:
                    match = True
        return match
